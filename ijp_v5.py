"""
IJP Dashboard AI Agent — ijp_v5.py (Production Grade)
======================================================

WHY ijp_v5.py EXISTS
--------------------
The original iv4.py had limitations similar to dv2.py:
1. No validation before DAX generation
2. Basic error handling without per-step repair
3. No workflow engine for complex queries
4. Manual filter building
5. No dependency resolution
6. Minimal documentation

WHAT CHANGED IN ijp_v5.py
--------------------------
- Full v12.py architecture with workflow engine
- Validation (measures + columns)
- Per-step repair loops
- Generic filter/DAX builders
...
Architecture:
  question
    → retrieve_ijp_measures
    → extract_ijp_intent  (classify: standard / comparison / topn / workflow)
         │
         ├─ standard / topn / comparison
         │     → build_ijp_dax
         │     → execute_ijp_dax ──(fail)──► repair_ijp_dax ──► execute
         │          │ (200)
         │     → generate_ijp_answer → END
         │
         └─ workflow
               → plan_ijp_workflow
               → execute_ijp_workflow  ←─────────────────────────────┐
                    │ step succeeded                                  │
                    │ step failed ──► repair_ijp_workflow_step ───────┘
                    │ all steps done
               → generate_ijp_answer → END
"""
...
def enrich_ijp_intent_node(state: IJPState) -> IJPState:
    ...
    if intent_type in ("standard", "topn", "comparison"):
        ...
        if measure_errors or column_errors:
            ...
            return {**state, "enriched_intent": intent, "error_messages": [error_msg],
...
def execute_ijp_dax_node(state: IJPState) -> IJPState:
    ...
    try:
        ...
        if state.get("repair_attempts", 0) < MAX_REPAIR_ATTEMPTS:
            return {**state, "execution_status": 500, "error_messages": [error_msg]}
        else:
            return {**state, "execution_status": 500, "error_messages": [f"Max retries. Error: {error_msg}"],
...
def repair_ijp_dax_node(state: IJPState) -> IJPState:
    ...
    error_msg = state.get("error_messages", ["Unknown"])[-1]
    ...
    return {**state, "final_dax": repaired_dax, "repair_attempts": state.get("repair_attempts", 0) + 1}
...
def plan_ijp_workflow_node(state: IJPState) -> IJPState:
    ...
    for step in workflow_plan:
        ...
        if measure_errors or column_errors:
            ...
            return {**state, "workflow_plan": [], "error_messages": [error_msg], "answer": error_msg}
