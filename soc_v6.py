"""
Span-of-Control (SOC) Analytics Agent — soc_v1.py
====================================================

WHY soc_v1.py EXISTS
--------------------
The original SOC agent had several structural problems:

1. DIMENSION_MAP was hardcoded to only 6 columns, so queries like "grouped
   by Level 1" silently broke (column not in map → ValueError).
2. Top-N and sort were parsed by IntentExtractor but NEVER applied in DAX —
   DaxBuilder.groupby_query() ignored intent.top_n and intent.sort_by
   completely, so "Top 10 managers by span" returned all managers unsorted.
3. No repair loop — executor.execute() called response.raise_for_status(),
   so any Power BI DAX error crashed the whole agent with a Python exception.
4. No answer generation — returned raw JSON, not a business-readable answer.
5. No validation — hallucinated measure/column names were never caught before
   the DAX was sent to Power BI.
6. Comparison queries ("Compare managers and employees") had a code path
   (execute_many) that was built but never actually called.
7. Layers / SELECTEDVALUE-dependent measures were treated identically to
   regular aggregatable measures, producing empty or wrong DAX.

WHAT CHANGED IN soc_v1.py
--------------------------
- Follows the same LangGraph node + state architecture as v6.py (HR agent).
- Column resolution auto-built from soc_dimensions.json — no hardcoded map.
  Resolves every column in Direct Reportee, Complete Span, and Dim Cal.
- TREATAS used for dimension-value filters (correct SOC model idiom);
  FILTER(ALL(...)) used for Dim Cal[Year] date filters (same as v6).
- Top-N correctly wrapped as TOPN(..., SUMMARIZECOLUMNS(...), sort_expr).
- Per-step repair loop exposed as a LangGraph edge — same pattern as v6's
  repair_dax_node, so DAX errors are retried up to MAX_REPAIR_ATTEMPTS.
- Answer generation via LLM — returns a business-readable answer.
- Validation against soc_measures.json + soc_dimensions.json before any
  DAX is generated, so hallucinated names are caught early.
- SELECTEDVALUE-dependent measures (Layers, Direct_Repotee_Count,
  Complete_Span, etc.) are detected and refused with a clear message rather
  than silently producing empty DAX.
- Comparison intent generates side-by-side ROW() with multiple measures,
  using the existing execute() path (not the broken execute_many path).

Architecture:
  question
    → retrieve_soc_measures
    → extract_soc_intent    (classify: standard / comparison / topn / workflow)
         │
         ├─ standard / topn / comparison
         │     → build_soc_dax
         │     → execute_soc_dax ──(fail)──► repair_soc_dax ──► execute_soc_dax
         │          │ (200)
         │     → generate_soc_answer → END
         │
         └─ workflow
               → plan_soc_workflow   (LLM plans ordered step list)
               → execute_soc_workflow  ←──────────────────────────────┐
                    │ step succeeded                                   │
                    │ step failed ──► repair_soc_workflow_step ────────┘
                    │ all steps done
               → generate_soc_answer → END

    Any node that catches an unrecoverable error routes to error_handler.
"""
